...
newtype Renaming = Renaming { unRenaming :: M.Map Name Name }
...
renameBinder :: IdSupply -> Renaming -> Name -> (IdSupply, Renaming, Name)
renameBinder ids rn n = (ids', insertRenaming n n' rn, n')
  where (ids', n') = freshName ids (name_string n)

renameBinders :: IdSupply -> Renaming -> [Name] -> (IdSupply, Renaming, [Name])
